...
public class MetaDataRegressionTest extends BaseTestCase {
    ...
    public void testBug8800() throws Exception {
        assertEquals(((JdbcConnection) this.conn).lowerCaseTableNames(), !this.conn.getMetaData().supportsMixedCaseIdentifiers());
        assertEquals(((JdbcConnection) this.conn).lowerCaseTableNames(), !this.conn.getMetaData().supportsMixedCaseQuotedIdentifiers());
    ...
    public void testBug9917() throws Exception {
        ...
        try {
            ...
            boolean defaultCatalogConfig = ((JdbcConnection) this.conn).getPropertySet()
    ...
    private void checkRsmdForBug13277(ResultSetMetaData rsmd) throws SQLException {
        ...
        int i = ((com.mysql.cj.jdbc.ConnectionImpl) this.conn).getSession().getServerSession().getMaxBytesPerChar(CharsetMapping
                .getJavaEncodingForMysqlCharset(((JdbcConnection) this.conn).getSession().getServerSession().getServerDefaultCharset()));
        if (i == 1) {
                    ...
                    if ("CHAR_OCTET_LENGTH".equals(metadataExpected.getColumnName(i + 1))) {
                        if (((com.mysql.cj.jdbc.ConnectionImpl) this.conn).getSession().getServerSession()
                                .getMaxBytesPerChar(CharsetMapping.getJavaEncodingForMysqlCharset(
                                        ((JdbcConnection) this.conn).getSession().getServerSession().getServerDefaultCharset())) > 1) {
        ...
        try {
            ...
            while (this.rs.next()) {
                ...
                assertTrue("Parameter " + columnName + " is not java.sql.DatabaseMetaData.procedureNullable.",
                        columnNullable.intValue() == DatabaseMetaData.procedureNullable);
            }
        } finally {
            if (con != null) {
                con.close();
    ...
    public void testBug35115() throws Exception {
        ...
        assertEquals("YEAR columns should be mapped to java.lang.Short", Short.class.getName(), rsMetaData.getColumnClassName(1));
        assertEquals("YEAR columns should be returned as java.lang.Short", Short.class.getName(), this.rs.getObject(1).getClass().getName());
