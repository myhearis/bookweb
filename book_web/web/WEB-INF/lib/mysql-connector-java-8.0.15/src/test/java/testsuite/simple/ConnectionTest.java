...
public class ConnectionTest extends BaseTestCase {
    ...
    public void testDumpQueriesOnException() throws Exception {
        ...
        try {
            ((JdbcConnection) dumpConn).clientPrepareStatement(bogusSQL).executeQuery();
        } catch (SQLException sqlEx) {
            assertTrue(sqlEx.getMessage().indexOf(bogusSQL) != -1);
        }
    ...
    public void testLocalInfileDisabled() throws Exception {
        ...
        try {
            ...
            ((JdbcConnection) loadConn).getPropertySet().getProperty(PropertyKey.allowLoadLocalInfile).setValue(false);
    ...
    public void testSetProfileSql() throws Exception {
        ((JdbcConnection) this.conn).getPropertySet().getProperty(PropertyKey.profileSQL).setValue(false);
        this.stmt.execute("SELECT 1");
        ((JdbcConnection) this.conn).getPropertySet().getProperty(PropertyKey.profileSQL).setValue(true);
        this.stmt.execute("SELECT 1");
    }
    ...
    private void testInterfaceImplementation(Connection connToCheck) throws Exception {
        ...
        pStmtToCheck = ((JdbcConnection) connToCheck).serverPrepareStatement("SELECT 1");
        ...
        checkInterfaceImplemented(ResultSetMetaData.class.getMethods(), toCheckRs.getMetaData().getClass(), toCheckRs.getMetaData());
